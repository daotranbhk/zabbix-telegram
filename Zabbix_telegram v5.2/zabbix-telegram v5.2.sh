...
# Date: 25/10/2017
# Update: 28/01/2021 zabbix 5.2 OK
...
DEBUG_FILE="/dev/null"
#DEBUG_FILE="/tmp/telegram.$(date "+%Y.%m.%d-%H.%M.%S").log"
#DEBUG_FILE2="/tmp/telegram.debug$(date "+%Y.%m.%d-%H.%M.%S").log"
...
# Zabbix address
#############################################
ZBX_URL="http://192.168.10.61/zabbix"
...
BOT_TOKEN="1444665402:AAEbo4C1T4bgVxHt2Q3XLRcNIvvzkuUZXWM"
...
if [ $(($SEND_GRAPH)) -eq '1' ]; then
	...
	if [ "${GRAPHID}" == "000001" ]; then
		GRAPHID="00002";
		${CURL} -k -s -S --max-time 5 -c ${COOKIE} -b ${COOKIE} -d "graphid=${GRAPHID}&from=${From}&to=${To}&width=${WIDTH}" ${ZBX_URL}"/chart2.php" -o "${PNG_PATH}";
		...
		if [ "${ZABBIXVERSION34}" == "1" ]; then
			...
			${CURL} -k -s -S --max-time 5 -c ${COOKIE}  -b ${COOKIE} -d "itemids=${GRAPHID}&from=${From}&to=${To}&width=${WIDTH}&profileIdx=web.charts.filter" ${ZBX_URL}"/chart.php" -o "${PNG_PATH}";
		else
			#${CURL} -k -s -S --max-time 5 -c ${COOKIE}  -b ${COOKIE} -d "itemids=${GRAPHID}&period=${PERIOD}&width=${WIDTH}" ${ZBX_URL}"/chart.php" -o "${PNG_PATH}";
			${CURL} -k -s -S --max-time 5 -c ${COOKIE}  -b ${COOKIE} -d "itemids=${GRAPHID}&from=${From}&to=${To}&width=${WIDTH}" ${ZBX_URL}"/chart2.php" -o "${PNG_PATH}";
		fi
	fi
	
	# Send the image to Telegram ${PERIOD}
#	if [ "$SEND_MESSAGE" -eq 1 ]
#		then
#       			${CURL} -k -s -S --max-time 5 -c ${COOKIE} -b ${COOKIE} --data-binary @${ZABBIXMSG} -X GET "https://api.telegram.org/bot${BOT_TOKEN}/sendMessage" >> $DEBUG_FILE
#	fi
